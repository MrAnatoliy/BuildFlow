...
async def publish_event(routing_key: str, payload: dict):
    ...
    logger.info(f'Recieved message publish request : service - {event_service} | event - {event_name}')
    logger.info(f'Message body : {payload}')
    connection = await aio_pika.connect_robust(settings.message_broker_url)
    logger.info(f'Connection with message broker at {settings.message_broker_url} has been established')
